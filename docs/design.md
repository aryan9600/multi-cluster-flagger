...
`spec.targetRef` and `spec.autoSclareRef` are references to the workload and its autoscaler. The workload and the autoscaler are expected to be present in every target cluster.
...
In the management cluster, users will install a new controller `multi-cluster-flagger` offering its own [API](#api) which is a wrapper around Flagger's Canary API.
**Note:** All the target/leaf clusters should have Flagger installed.

Applying the above MultiClusterCanary in the management cluster, will lead to the below Canary being applied in a target cluster:
  ...
  analysis:
    ...
    webhooks:
     ...
     - name: "confirm rollout"
       ...
       url: http://multi-cluster-flagger/confirm-rollout
     - name: "promotion gate"
       type: confirm-promotion
       url: http://multi-cluster-flagger/confirm-promotion
     - name: "rollback gate"
       type: rollback
       url: http://fleet-flagger.com/rollback
```
...
* Once MCF has received a webhook call for each required Canary, it'll send a response with a 2xx status code, allowing the Flagger instances to start shifting traffic to the canary deployment.
...
* If there are Canary objects that failed, then MCF will [retry](https://fluxcd.io/flagger/faq/#how-to-retry-a-failed-release) them. Meanwhile, it'll keep returning a non-2xx response to the Flagger instances reconciling the passing Canaries.
